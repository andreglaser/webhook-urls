...
app.post('/get-final-url', async (req, res) => {
  ...
  try {
    ...
    // Capturar URL final da navegação da página
    const pageUrl = page.url();
    
    // Determinar a URL final REAL:
    // 1. Se temos redirects, o último redirect é a URL final real
    // 2. Se não temos redirects, usar a URL da página
    if (redirects.length > 0) {
      const lastRedirect = redirects[redirects.length - 1];
      finalUrl = lastRedirect.to;
    } else {
      finalUrl = pageUrl;
    }
    ...
    // Por padrão, usar a URL final real
    let bestUrl = finalUrl;
    
    // Se a URL final é de login e temos uma searchUrl válida, usar a searchUrl
    if (searchUrl && (finalUrl.includes('login') || finalUrl.includes('signin') || finalUrl.includes('auth'))) {
      bestUrl = searchUrl;
      console.log(`Usando searchUrl como recomendada (finalUrl é página de login)`);
    }
    ...
    res.json({ 
      success: true,
      originalUrl: url,
      finalUrl: finalUrl, // Sempre a URL de destino final real
      searchUrl: searchUrl || null,
      recommendedUrl: bestUrl, // URL recomendada (evita login quando possível)
      pageUrl: pageUrl, // URL onde a página parou de navegar
      redirects: redirects,
